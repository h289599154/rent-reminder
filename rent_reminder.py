...
def gen_doc_html(doc_name, color, overdue_list, today_list):
    ...
    <div style="margin-bottom:12px">
        <h3 style="font-size:16px;margin:0 0 8px;padding-left:6px;border-left:3px solid {c['border']};color:{c['title']}">{doc_name}</h3>
...
def gen_combined_html(doc_keys, all_data):
    ...
    for key in doc_keys:
        ...
        if overdue or today:
            doc = DOCUMENTS[key]
            html = gen_doc_html(doc["name"], doc["color"], overdue, today)
            if html:
                blocks.append(html)
...
def main():
    try:
        # 读取所有文档，存储为 {key: (overdue_list, today_list)}
        all_data = {}
